...
var FilterPanel = Backbone.View.extend({
	...
	initialize : function(config){
		...
		this.priceRanges = config.priceRanges;
		...
		this.template = _.template([
			...
			'<%_.each(categories,function(category){print("<li><a class=\'cat cat1\' data-category=\'" + category.id + "\'><span></span><div>" + category.category + "</div></a></li>");});%>',
			'</ul>',
			'</div>',
			'</li>',
			'<li>',
			...
			'<%_.each(priceRanges,function(range,index){print("<li><a class=\'cat1\' data-category=\'" + index + "\'><span></span><div>" + range.start_price + (range.end_price ? "-" + range.end_price : "+") + "</div></a></li>");});%>',
			'</ul>',
			'</div>',
			'</li>',
			'</ul>',
	...
	addFilter : function(event,el){
		...
		if($(el).hasClass('cat')){
			//filter by category
			that.category_id = parseInt($(el).attr("data-category"));
			$.ajax({
				url : 'data',
				data : {
					category_id : that.category_id || 0,
					price_range : that.price_range || 0
				},
			...
			that.price_filter = parseInt($(el).attr("data-category"));
			//=====dead code
			...
			$.ajax({
				...
				data : {
					category_id : that.category_id || 0,
					price_filter : that.price_filter || 0
				},
				success : function(res){
					data = that.processResponse(res);
					app.getStore().add(data);
			...
			while(len--){
				...
				for(var i = 0; i < that.filters.length; i++ ){
					if(that.filters[i].type === "category"){
						...
						if(item.get("discount_price") < (that.priceRanges[that.filters[i].filterValue].end_price || 1000000) && item.get("discount_price") > that.priceRanges[that.filters[i].filterValue].start_price){
							hasCategory++;
						}
					}
...
$(function(){
	...
	app = (function(config){
		...
		categories : $.parseJSON($('#categories').val()),
		priceRange : $.parseJSON($('#priceRange').val())
	});
