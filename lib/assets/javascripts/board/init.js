			...
			while(len--){
				...
				for(var i = 0; i < that.filters.length; i++ ){
					if(that.filters[i].type === "category"){
						...
						if(item.get("discount_price") < that.priceRanges[that.filters[i].filterValue].end_price && item.get("discount_price") > that.priceRanges[that.filters[i].filterValue].start_price){
							hasCategory++;
						}
					}
