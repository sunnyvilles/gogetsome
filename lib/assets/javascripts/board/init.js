...
var FilterPanel = Backbone.View.extend({
	...
	initialize : function(config){
		...
		this.template = _.template([
			...
			'<li><a class="cat cat1" data-category="7"><span></span><div>Shoes</div></a></li>',
			'<li><a class="cat cat2" data-category="4"><span></span><div>Men</div></a></li>',
			'<li><a class="cat cat3" data-category="22"><span></span><div>Casual</div></a></li>',
			'<li><a class="cat cat4" data-category="43"><span></span><div>Sandals</div></a></li>',
			'<li><a class="cat cat5" data-category="2"><span></span><div>Sports Wear</div></a></li>',
			'<li><a class="cat cat6" data-category="9"><span></span><div>Puma</div></a></li>',
			'<li><a class="cat cat7categorie" data-category="15"><span></span><div>Black</div></a></li>',
			'<li><a class="cat cat7" data-category="25"><span></span><div>Adidas</div></a></li>',
			'</ul>',
			'</div>',
			'</li>',
			'<li>',
		...
		setTimeout(function(){
			...
			for(var i=0,len=app.getStore().length;i<len;i++){
				...
				var item = new PictureTile({
					...
					attributes : {
						id : model.get('id'),
						price : model.get('actual_price'),
						popularityIndex : model.get('discount')
					}
				});
				item.render();
				$('.mainWrapper').append(item.el);
...
var PictureTile = Backbone.View.extend({
	...
	initialize : function(config){
		this.template = _.template([
			...
			'<div class="floatLeft">',
			'<span class="discountedPrice red">Rs. <%=actual_price%> <span class="strike gray originalPrice"><%=discount_price%></span></span>',
			'<div class="perOff red fontBold">(<%=((actual_price && discount_price) ? Math.floor(((discount_price-actual_price)/discount_price)*100): 0)%>% OFF)</div>',
			'</div>',
			...
			'<div class="floatLeft">',
			'<div class="perOff red fontBold" style="font-size:11px;">(<%=((actual_price && discount_price) ? Math.floor(((discount_price-actual_price)/discount_price)*100): 0)%>% OFF) Rs. <%=actual_price%> </div>',
			'</div>',
