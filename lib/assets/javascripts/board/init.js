...
var FilterPanel = Backbone.View.extend({
	...
	initialize : function(config){
		...
		this.template = _.template([
			...
			'<%_.each(categories,function(category){print("<li><a class=\'cat cat1\' data-category=\'" + category.id + "\'><span></span><div>" + category.name + "</div></a></li>");});%>',
			'</ul>',
			'</div>',
			'</li>',
			'<li>',
			...
			'<%_.each(priceRanges,function(range){print("<li><a class=\'cat cat1\' data-category=\'" + range.id + "\'><span></span><div>" + range.name + "</div></a></li>");});%>',
			'</ul>',
			'</div>',
			'</li>',
			'</ul>',
