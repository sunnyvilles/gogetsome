...
var PictureWall = Backbone.View.extend({
	...
	populateWall : function(products,indexed_sites){
		...
		setTimeout(function(){
			...
			console.log(app.getStore().length);
			for(var i=0,len=app.getStore().length;i<len;i++){
				...
				var item = new PictureTile({
					...
					attributes : {
						...
						popularityIndex : model.get('popularityIndex')
					}
				});
				item.render();
				$('.mainWrapper').append(item.el);
...
var PictureTile = Backbone.View.extend({
	...
	initialize : function(config){
		this.template = _.template([
			...
			'<div class="perOff red fontBold">(<%=actual_price%>% OFF)</div>',
			'</div>',
      ...
      '<div class="floatLeft">',
      '<div class="perOff red fontBold" style="font-size:11px;">(10% OFF) Rs. <%=discount_price%> </div>',
      '</div>',
