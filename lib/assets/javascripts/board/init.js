...
var FilterPanel = Backbone.View.extend({
	...
	initialize : function(config){
		...
		this.template = _.template([
			...
			'<%_.each(priceRanges,function(range){print("<li><a class=\'cat cat1\' data-category=\'" + range.id + "\'><span></span><div>" + range.start_price + (range.end_price ? "-" + range.end_price : "+") + "</div></a></li>");});%>',
			'</ul>',
			'</div>',
			'</li>',
			'</ul>',
	...
	addFilter : function(event,el){
		...
		if($(el).hasClass('cat')){
			...
			that.price_range = parseInt($(el).attr("data-category"));
			//=====dead code
