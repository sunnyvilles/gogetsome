  ...
  def self.get_categories_for_sort
    ...
    if categories.nil?
      ...
      filter_categories.each do |category|
        associated_categories[category.id], sub_categories = [], []
        ...
        single_associated_categories.each do |cat|
          sub_categories << cat.parent_category_id if cat.parent_category_id != category.id
          sub_categories << cat.child_category_id if cat.child_category_id != category.id
        end
        Category.where(:id => sub_categories).each do |category|
